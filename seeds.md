# seeds

> Not yet canon. Dated, terse. Promote to `direction.md` only when Joel confirms.

## Open

- **Lane (2026-09-06)** — traditional+motion (Loft 31, DFD) vs ambitious camera (Faers, Portale reel). Recommendation: stay on `/vision` as one continuous land. Steal their restraint and film-as-weather, not brochure IA. Notes: `moodboard/refs.md`.
- **Remaining vision beats** — hard land; ranch / school / retreat apron geometry; close (`The land becomes the institution.` + valle.land). First pass is 1–4 + thesis only.
- **Splash field (2026-09-07)** — dusk mycelium + outward rays behind the lockup; pointer displaces, then springs back. Not the chingón settle mechanic.
- **Type** — splash type lives in the still. Vision captions use a system serif until a study lands.
- **Sound** — later. Not on `/`.
- **Domains** — valle.land (Spaceship) + valle.la (Porkbun) bought 2026-09-06. land → Pages; la 301 → land.

Apex `valle.land` → GitHub Pages (same IPs as chingón). Leave any MX/TXT alone if they appear.

```
A      @     185.199.108.153
A      @     185.199.109.153
A      @     185.199.110.153
A      @     185.199.111.153
AAAA   @     2606:50c0:8000::153
AAAA   @     2606:50c0:8001::153
AAAA   @     2606:50c0:8002::153
AAAA   @     2606:50c0:8003::153
CNAME  www   joelrojo.github.io
```

`www.valle.land` 301s to apex (GitHub). HTTPS enforced 2026-09-07 after reset-cname minted the cert. GitHub is not sending HSTS on the custom domain, so Chrome can stay on `http://valle.land` (Not Secure + “Certificate is valid”). `index.html` upgrades that hop in-page.

`valle.la` 301s to `https://valle.land` (Porkbun URL forward, wildcard + path, set 2026-09-07). Do not put Pages records on `.la`.
- **GoatCounter** — later. Not now. Paths `/` vs `/vision` are why, if we add it. Not Counter.dev. No Google.
- **Repo visibility** — public so GitHub Pages works on a free plan (same as chingón). The deck will be shared anyway.

## Decided (2026-09-07)

- Share / meta: `Tierra meets technology. A living system grown in public at the edge of Los Angeles.`

## Decided (2026-09-06)

- Place name: Valle del Sol. Doorway: valle.land + valle.la. Tierra Sol set aside.
- `/` is the still lockup. `/vision` is the first camera-scroll.
- Stack: Vite + React + R3F + GSAP ScrollTrigger.
- Host: GitHub Pages from `dist/` on `joelrojo/sol`.
